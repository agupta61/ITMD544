...
# $2 would reference the second value ambujkey and so forth...
...
aws rds create-db-instance --db-instance-identifier FA5CLOUDDB --allocated-storage 5 --db-instance-class db.t2.micro --port 3306 --multi-az --engine MySQL --engine-version 5.6.19a --master-username userambuj --master-user-password usergupta
...
aws sns subscribe --topic-arn $arn --protocol email --notification-endpoint agupta61@hawk.iit.edu
...
aws rds create-db-instance-read-replica --db-instance-identifier ReadFACLOUDDB --source-db-instance-identifier FA5CLOUDDB --db-instance-class db.t2.micro --port 3306 
#rm filepush.sh
